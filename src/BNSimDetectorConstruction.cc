...
G4VPhysicalVolume* BNSimDetectorConstruction::Construct()
{
	...
	if (detectorMaterial != "AIR")
		...
		ConstructEmptyDetector(logicWorld);
...
void BNSimDetectorConstruction::ConstructDetector(G4LogicalVolume* logicWorld)
{
	G4double z, a, density;
	G4String name, symbol;
	G4int numberofelements, numberofatoms;
...
void BNSimDetectorConstruction::ConstructEmptyDetector(G4LogicalVolume* logicWorld)
{
	// Get nist material manager
	G4NistManager* nist = G4NistManager::Instance();
	...
	G4VPhysicalVolume* physDetector =
		new G4PVPlacement(0, G4ThreeVector(0., 0., -0.5 * (dtctrz + sourceDetectorDistance)), logicEmptyDetector, "EmptyDetector", logicWorld, false, 0, checkOverlaps);
